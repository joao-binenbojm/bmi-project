classdef ldaClassifier < handle
    ...
    methods
        ...
        function [obj] = fit(obj,trainingData,dt)
            ...
            [~,~,X] = obj.fr_features(trainingData,dt,N); % obtaining firing rate feature space from training data
        ...
        function [out,obj] = predict(obj,testData,dt)
            %PREDICT(testData,N) uses trained model to generate labels on
            %test data
            
            N = length(testData.spikes);
            [~,~,X] = obj.fr_features(testData,dt,N); % preprocess EEG data
            X = (X-obj.fr_norm.mean)./obj.fr_norm.std;
            X(isnan(X)) = 0;
            X(isinf(X)) = 0;
            X = X*obj.P;
